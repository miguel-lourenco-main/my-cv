 ...
 *
 * Next.js injects `assetPrefix` into `next/image` and framework requests, but
 * NOT into Three.js loaders (`TextureLoader`, drei `useTexture`/`useGLTF`) or
 * raw `<img>`. Route every WebGL/canvas asset URL through this helper so they
 * resolve correctly under a subpath deployment.
 *
